import { ClassSel, playerSelections, classSelHelperTD } from "@/controllers/classSel";
...
PlayerEvent.onText(({ player, text, next }) => {
  ...
  Player.sendClientMessageToAll(ColorEnum.White, `${player.getName().name}(${player.id}): ${text}`);
  next();
  return false;
});
...
PlayerEvent.onCommandError(({ player, command, error, next }) => {
  player.sendClientMessage(
    ColorEnum.White,
    $t(`error.command${error.code ? "Undefined" : "Format"}`, [command], player.locale),
  );
  next();
  return true;
});
