...
export const loadAllStaticVehicles = async () => {
  try {
    const promiseList = vehicleFiles.map((fileName) => {
      return LoadStaticVehiclesFromFile(path.join(vehicleFolder, fileName + ".txt"));
    });
    ...
    vehicleDataList.forEach((vehicles: string[]) => {
      ...
      vehicles.forEach((vehicle: string) => {
        ...
        const [modelId, spawnX, spawnY, spawnZ, spawnRot, color1, color2] = realData;
        ...
        if (veh.id !== -1) {
          ...
          logger.error("Failed to create vehicle", realData, vehicleFiles[fileIndex], index);
        }
      });
      fileIndex++;
    });
